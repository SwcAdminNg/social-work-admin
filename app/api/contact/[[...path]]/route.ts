...
async function forward(
  ...
  if (session.user.userType !== "ADMIN") {
    return NextResponse.json(
      {
        success: false,
        message: "You do not have permission to manage messages",
        errors: null,
      },
      { status: 403 },
    );
  ...
  const segments = path ?? [];
  const backendPath = `/contact-us${segments.length ? "/" + segments.join("/") : ""}${request.nextUrl.search}`;
  ...
  try {
    ...
    return NextResponse.json(
      {
        success: false,
        message: "Unexpected error contacting the service",
        errors: null,
      },
      { status: 500 },
    );
