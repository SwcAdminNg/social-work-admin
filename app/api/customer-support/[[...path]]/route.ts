...
async function forward(
  ...
  if (session.user.userType !== "ADMIN") {
    return NextResponse.json(
      {
        success: false,
        message: "You do not have permission to manage customer support",
        errors: null,
      },
      { status: 403 },
    );
  ...
  const segments = path ?? [];
  const backendPath = `/customer-support${segments.length ? "/" + segments.join("/") : ""}${request.nextUrl.search}`;
  ...
  try {
    ...
    return NextResponse.json(
      {
        success: false,
        message: "Unexpected error contacting the customer support service",
        errors: null,
      },
      { status: 500 },
    );
