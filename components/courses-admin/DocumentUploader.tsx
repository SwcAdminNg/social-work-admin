...
import { finalizeDocument } from "@/lib/api/courses-client";
import type { CourseDocument, CourseItem, DocumentUploadCredentials } from "@/lib/api/courses.types";
...
export function DocumentUploader({
  ...
  onDocumentUpdate: (document: CourseDocument) => void;
}) {
  ...
  async function handleFileSelected(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      await finalizeDocument(item.id, { mime_type: file.type, file_size_bytes: file.size });
      onDocumentUpdate({
  ...
  if (!credentials) {
    return (
      <p className="text-sm text-amber-700 dark:text-amber-400 bg-amber-50 dark:bg-amber-500/10 rounded-xl px-3.5 py-2.5">
        The upload link for this document has expired (10 minute window). Delete this item and create it
