...
import { refreshVideoUpload } from "@/lib/api/courses-client";
import type { CourseItem, CourseVideo } from "@/lib/api/courses.types";
...
export function VideoUploader({
  ...
  item: CourseItem;
  onVideoUpdate: (video: CourseVideo) => void;
  ...
  const status = item.video?.status ?? "PENDING";
  ...
  async function handleFileSelected(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    setPreparing(true);
    try {
      const credentials = await refreshVideoUpload(item.id);
      setPreparing(false);
      setProgress(0);
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      {status === "READY" && item.video?.playback_url && (
        <video
          controls
          className="w-full max-w-md rounded-xl"
          src={item.video.playback_url}
