...
import { getThumbnailUploadUrl } from "@/lib/api/resources-client";
...
export function ThumbnailUploader({
  ...
  async function handleFileSelected(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    setUploading(true);
    try {
      const { upload_url, thumbnail_url } = await getThumbnailUploadUrl(resourceId, {
        file_name: file.name,
  ...
  return (
    <div>
      <label htmlFor="resource-thumbnail" className="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2">
        Thumbnail
      </label>
